...
public class HollowPrefixIndexTest {
    ...
    public void testCustomPrefixIndex() throws Exception {
        ...
        Assertions.assertTrue(ordinals.size() == 1);
        ...
        Assertions.assertTrue(ordinals.size() == 0);
    }
    ...
    public void testDeltaChange() throws Exception {
        ...
        Assertions.assertTrue(ordinals.size() == 1);
        Assertions.assertTrue(movieNames.contains("龍爭虎鬥"));


        ordinals = toSet(prefixIndex.findKeysWithPrefix("00"));
        movieNames = getMovieNames(ordinals, "SimpleMovie", "name");
        Assertions.assertTrue(ordinals.size() == 1);
        Assertions.assertTrue(movieNames.contains("007 James Bond"));
        ...
        movieNames = getMovieNames(ordinals, "SimpleMovie", "name");
        Assertions.assertTrue(ordinals.size() == 1);
        Assertions.assertTrue(movieNames.contains("As Good as It Gets"));

        ordinals = toSet(prefixIndex.findKeysWithPrefix("R"));
        movieNames = getMovieNames(ordinals, "SimpleMovie", "name");
        Assertions.assertEquals(ordinals.size(), 2);
        Assertions.assertTrue(movieNames.contains("Rocky 2"));
        Assertions.assertTrue(movieNames.contains("Rush"));
        ...
        movieNames = getMovieNames(ordinals, "SimpleMovie", "name");
        Assertions.assertTrue(ordinals.size() == 2);
        Assertions.assertTrue(movieNames.contains("007 James Bond"));
        Assertions.assertTrue(movieNames.contains("0 dark thirty"));
    ...
    public void testMovieActorReference_duplicatesInList() throws Exception {
        ...
        StateEngineRoundTripper.roundTripSnapshot(writeStateEngine, readStateEngine);
        HollowPrefixIndex prefixIndex = new HollowPrefixIndex(readStateEngine, "MovieActorReference", "actors.element");
        Assertions.assertEquals(numMovies, toSet(prefixIndex.findKeysWithPrefix("kea")).size());
    }
    ...
    private void test(List<Movie> movies, String type, String fieldPath) throws Exception {
        ...
        Assertions.assertEquals(ordinals.size(), 1);

        ordinals = toSet(prefixIndex.findKeysWithPrefix("ttt"));
        Assertions.assertEquals(ordinals.size(), 0);
