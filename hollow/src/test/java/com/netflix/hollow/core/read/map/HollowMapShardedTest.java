...
public class HollowMapShardedTest extends AbstractStateEngineTest {
    ...
    public void testShardedData() throws IOException {
        ...
        Assertions.assertEquals(8, readStateEngine.getTypeState("TestMap").numShards());
        
        HollowMapTypeDataAccess mapDataAccess = (HollowMapTypeDataAccess) readStateEngine.getTypeDataAccess("TestMap");
        for(int i=0;i<1000;i++) {
            Assertions.assertEquals(3, mapDataAccess.size(i));
            Assertions.assertEquals(i+1, mapDataAccess.get(i, i));
            Assertions.assertEquals(i+3, mapDataAccess.get(i, i+2));
            Assertions.assertEquals(i+5, mapDataAccess.get(i, i+4));
        }
        ...
        while(ordinal != -1) {
            Assertions.assertEquals(3, mapDataAccess.size(ordinal));
            Assertions.assertEquals(expectedValue+1, mapDataAccess.get(ordinal, expectedValue));
            Assertions.assertEquals(expectedValue+3, mapDataAccess.get(ordinal, expectedValue+2));
            Assertions.assertEquals(expectedValue+5, mapDataAccess.get(ordinal, expectedValue+4));
