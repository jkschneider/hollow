...
public class MissingMapTest extends AbstractStateEngineTest {
    ...
    public void testCompletelyMissingMap() throws IOException {
        ...
        Assertions.assertTrue(map.containsKey(new FakeMissingHollowRecord(new HollowObjectMissingDataAccess(readStateEngine, "MissingObject"), 2)));
        Assertions.assertEquals(300, map.get(new FakeMissingHollowRecord(new HollowObjectMissingDataAccess(readStateEngine, "MissingObject"), 3)).getOrdinal());
        Assertions.assertFalse(map.containsKey(new FakeMissingHollowRecord(new HollowObjectMissingDataAccess(readStateEngine, "MissingObject"), 0)));
        Assertions.assertNull(map.get(new FakeMissingHollowRecord(new HollowObjectMissingDataAccess(readStateEngine, "MissingObject"), 4)));
        ...
        Assertions.assertEquals(2, next.getKey().getOrdinal());
        Assertions.assertEquals("MissingObject", next.getKey().getSchema().getName());
        Assertions.assertEquals(200, next.getValue().getOrdinal());
        Assertions.assertEquals("MissingObject", next.getValue().getSchema().getName());
        Assertions.assertTrue(rec.hasNext());
        next = rec.next();
        Assertions.assertEquals(3, next.getKey().getOrdinal());
        Assertions.assertEquals("MissingObject", next.getKey().getSchema().getName());
        Assertions.assertEquals(300, next.getValue().getOrdinal());
        Assertions.assertEquals("MissingObject", next.getValue().getSchema().getName());
        Assertions.assertEquals(300, map.get(next.getKey()).getOrdinal());
        Assertions.assertEquals("MissingObject", map.get(next.getKey()).getSchema().getName());
