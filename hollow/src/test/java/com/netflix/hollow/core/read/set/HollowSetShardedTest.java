...
public class HollowSetShardedTest extends AbstractStateEngineTest {
    ...
    public void testShardedData() throws IOException {
        ...
        Assertions.assertEquals(4, readStateEngine.getTypeState("TestSet").numShards());
        
        HollowSetTypeDataAccess setDataAccess = (HollowSetTypeDataAccess) readStateEngine.getTypeDataAccess("TestSet");
        for(int i=0;i<1000;i++) {
            Assertions.assertEquals(3, setDataAccess.size(i));
            Assertions.assertTrue(setDataAccess.contains(i, i));
            Assertions.assertTrue(setDataAccess.contains(i, i+1));
            Assertions.assertTrue(setDataAccess.contains(i, i+2));
        }
        ...
        while(ordinal != -1) {
            Assertions.assertEquals(3, setDataAccess.size(ordinal));
            Assertions.assertTrue(setDataAccess.contains(ordinal, expectedValue));
            Assertions.assertTrue(setDataAccess.contains(ordinal, expectedValue+1));
            Assertions.assertTrue(setDataAccess.contains(ordinal, expectedValue+2));
