...
public class RestoreWriteStateEngineSetTest extends AbstractStateEngineTest {
    ...
    public void test() throws IOException {
        ...
        Assertions.assertEquals(4, readStateEngine.getTypeState("TestSet").maxOrdinal());
        assertSet(0, 1, 2, 3);
        assertSet(1, 4, 5, 6);
        ...
        Assertions.assertEquals(3, readStateEngine.getTypeState("TestSet").maxOrdinal());
    }
    ...
    private void assertSet(int ordinal, int... elements) {
        ...
        Assertions.assertEquals(elements.length, readState.size(ordinal));
        ...
        for(int element : elements) {
            if(!readState.contains(ordinal, element, element + 10)) { // the hash code is deliberately specified here as different from the ordinal.
                Assertions.fail("Set did not contain element: " + element);
            }
        }
    }
